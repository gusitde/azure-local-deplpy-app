"""Install Azure Local OS on servers .6 and .7 via iDRAC virtual media + CIFS (fixed creds)."""
import requests
import urllib3
import time
...
def install(ip, name):
    ...
    # 4. Mount ISO via CIFS with DOMAIN\user credentials
    ...
    r = rf(ip, "/redfish/v1/Managers/iDRAC.Embedded.1/VirtualMedia/CD")
    d = r.json()
    print(f"  Verify: Inserted={d.get('Inserted')}, ConnectedVia={d.get('ConnectedVia')}")
    ...
    if r.status_code not in (200, 204):
        print(f"  Boot override failed: {r.status_code}")
    else:
        print(f"  Boot override set OK")
...
if __name__ == "__main__":
    ...
    # Cancel any stale SimpleUpdate job from the test
    # (Not critical, just clean)
    ...
    print(f"Monitor with: ping 192.168.1.x to find new DHCP hosts")
    print(f"{'='*50}")
